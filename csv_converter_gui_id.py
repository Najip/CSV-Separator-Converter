...
class ConfirmDialog(tk.Toplevel):
    ...
    def initialize_dialog(self):
        ...
        tk.Button(btn_frame, text="Ganti", command=lambda: self.user_choice("replace")).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Batal", command=lambda: self.user_choice("cancel")).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Simpan dgn Nomor", command=lambda: self.user_choice("rename")).pack(side=tk.LEFT, padx=10)
...
class CSVSeparatorConverterApp:
    ...
    def initialize_gui(self):
        self.root.title('Konverter Separator CSV')
        self.root.iconbitmap(self.get_icon_path())

        tk.Label(self.root, text='Pilih File:').grid(row=0, column=0, sticky='w', pady=2)
        self.file_entry = tk.Entry(self.root, width=70)
        self.file_entry.grid(row=0, column=1, padx=5, pady=2, columnspan=3)
        tk.Button(self.root, text='Jelajahi', command=self.browse_file).grid(row=0, column=4, padx=5, pady=2)

        self.toggle_location_button = tk.Checkbutton(self.root, text='Pilih Lokasi Lain', command=self.toggle_location)
        self.toggle_location_button.grid(row=1, column=0, columnspan=5, sticky='w', pady=2)

        tk.Label(self.root, text='Pilih Lokasi Tujuan File:').grid(row=2, column=0, sticky='w', pady=2)
        ...
        self.browse_output_button = tk.Button(self.root, text='Jelajahi', command=self.browse_output, state='disabled')
        self.browse_output_button.grid(row=2, column=4, padx=5, pady=2)

        self.convert_button = tk.Button(self.root, text='Konversi', command=self.convert_file, state='disabled')
    ...
    def detect_separator(self):
        with open(self.input_file_path, 'r', encoding='utf-8') as file:
            ...
            if ',' in first_line:
                ...
                self.separator_notification_label.config(text='Pemisah "," terdeteksi. Tekan tombol "Konversi" untuk mengkonversi menjadi ";".')
            elif ';' in first_line:
                self.detected_separator = ';'
                self.separator_notification_label.config(text='Pemisah ";" terdeteksi. Tekan tombol "Konversi" untuk mengkonversi menjadi ",".')
    ...
    def convert_file(self):
        if not self.input_file_path:
            self.notification_label_text.config(text='Peringatan: Pilih file CSV terlebih dahulu!')
            return
        ...
        if os.path.exists(self.output_file_path):
            message = f"Nama file \"{output_file_name}\" sudah ada. Apakah anda yakin ingin mengganti file yang ada?"
            ConfirmDialog(self.root, "Konfirmasi", message, self.handle_user_choice)
        else:
    ...
    def write_to_csv(self, output_file_name):
        try:
            ...
            self.notification_label_text.config(text='Konversi Berhasil! File output disimpan di: ')
            self.notification_label_link.config(text=f'{self.output_file_path}')
            self.open_file_location(self.output_file_path)
        except Exception as e:
            messagebox.showerror("Error", "Ganti gagal! File sedang dibuka aplikasi lain.")
